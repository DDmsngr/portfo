...
### Шаг 2 — Поправь vite.config.js (только если без кастомного домена)

В этом проекте используется кастомный домен `deepdrift.tech` (см. `public/CNAME`), поэтому `base: '/'`.

Если переносишь проект на репозиторий без домена, замени на имя репо:
